              ...
              <table id="example4" class="table table-bordered table-hover">
                ...
                <tbody>
                  @foreach ( $data as $key => $value )
                    <tr>
                      <td>{{ $key + 1 }}</td>
                      <td>{{ $value['iou_title']}}</td>
                      <td>{{ $value['iou_created']}}</td>
                      <td><span class="label label-danger">{{ $value['iou_status'] }}</span></td>
                      <td><a class="btn btn-info" href="{{ url('/')}}/adjuster/iou/show/{{$value['iou_id'] }}">Detail</a></td>
                    </tr>
                  @endforeach
                </tbody>
              </table>
